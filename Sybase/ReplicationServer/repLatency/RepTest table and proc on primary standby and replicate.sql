...
LOCK ALLPAGES
go
GRANT SELECT ON dbo.RepTest TO read_un_limited
go
...
go
CREATE UNIQUE NONCLUSTERED INDEX XPKRepTest
    ON dbo.RepTest(repTestId)
go
IF EXISTS (SELECT * FROM sysindexes WHERE id=OBJECT_ID('dbo.RepTest') AND name='XPKRepTest')
    PRINT '<<< CREATED INDEX dbo.RepTest.XPKRepTest >>>'
ELSE
    PRINT '<<< FAILED CREATING INDEX dbo.RepTest.XPKRepTest >>>'
go
...
go
GRANT EXECUTE ON dbo.wsp_newRepTest TO webmaint
go
