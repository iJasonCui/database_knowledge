...
# Notes
#
# This script retrieves SCB Unbillable call information to transfer to SCB vendor 
# only fornformation. The SCB calls are loaded into two separate databases. 
# canNH for Canadian SCB calls and usaNH for US SCB calls. The DB server is v151dbp01ivr 
#
...
# ------------------------------------------------------------------------------------------------
# Jason		May 3 2007      initilization script	
#                               

# check number of parameters
if [ $# -lt 2 -o $# -gt 4 ] ; then
    echo ""
	echo "Usage: ${0} <out_server> <ISDorCADFlag> <from_date> <to_date>"
    echo "where:"
    echo "  out_server   - required, name of the server contained call records to xfer;"
...
FTP_TABLE="FtpSCBUnbillableRecords"
FTP_TABLE_VIEW="FtpSCBUnbillableCallId"
FTP_ARCHIVE_TABLE="FtpSCBUnbillableSent"
...
SENTFILE="${TODAYS_DATE}.SCB.${COUNTRY_ID}.unbillable.ftp"
SCP_FILE="${OUT_DIR}/${SENTFILE}"
BCP_FILE="${OUT_DIR}/${SENTFILE}"
BCP_FILE_VIEW="${OUT_DIR}/${SENTFILE}.bcp"
SQL_FILE="${OUT_DIR}/${SENTFILE}.sql"
OUT_FILE="${OUT_DIR}/${SENTFILE}.out"
TMP_FILE="${OUT_DIR}/${SENTFILE}.tmp"
MAIL_FILE="${OUT_DIR}/${SENTFILE}.mail"
...
# create mail file
echo "$0 started copy of SCB Unbillable call records at `date`" > ${MAIL_FILE}
...
use ${ARCHIVE_DB} 
go
update dbo.FtpSCBBatchUnbillable 
set batchId = batchId + 1
go
select max(batchId) from dbo.FtpSCBBatchUnbillable 
go
exit  
EOF1`
...
from    dbo.FtpSCBBatchLogUnbillable
...
if [ ${ERROR_CODE} -eq 0 ]; then
	echo "succeeded at `date`."                >> ${MAIL_FILE}
        ...
        echo '================================================' >> ${MAIL_FILE}
        echo ''                                    >> ${MAIL_FILE}
        echo 'Extract SCB unbillable FTP Records FROM  : ' ${LASTRUN_DATE} >> ${MAIL_FILE}
        echo 'Extract SCB unbillable FTP Records TO    : ' ${TO_DATE}      >> ${MAIL_FILE}
        ...
        echo "Step 2a - insert FtpSCBBatchLogUnbillable record...              " >> ${MAIL_FILE}
        # ... compose sql script
        echo "use ${ARCHIVE_DB}                                                "  > $SQL_FILE
        echo 'go                                                               ' >> $SQL_FILE
        echo 'exec p_insert_FtpSCBBatchLogUnbill '${BATCH_ID} ",'${TODAYS_DATE}','${LASTRUN_DATE}','${TO_DATE}','Started'," \
                   0,NULL,0,NULL,0,"'${SENTFILE}'," ${SITEID} >> $SQL_FILE
        echo 'go                                                               ' >> $SQL_FILE
        # ... run sql
	run_sql isql ${ARCHIVE_SERVER} ${ARCHIVE_DB} cron_sa ${ARCHIVE_PASSWORD} \
        ...
        if [ ${ERROR_CODE} -ne 0 ]; then
           ERROR_CODE=999
           echo "... Failed inserting record into FtpSCBBatchLogUnbillable table, error ${ERROR_CODE}." >> ${MAIL_FILE}
        fi
...
if [ ${ERROR_CODE} -ne 0 ]; then
    echo "Step 3 - truncate and reload table based on FROM/TO dates, skipped." >> ${MAIL_FILE}
        ...
        echo 'go                                                               ' >> $SQL_FILE
        echo 'Truncate table dbo.FtpSCBUnbillable                              ' >> $SQL_FILE
        echo 'go                                                               ' >> $SQL_FILE
        echo 'insert dbo.FtpSCBUnbillable                                      ' >> $SQL_FILE
        ...
        echo ' where  Call.siteId    = '${SITEID} 'and                         ' >> $SQL_FILE
...
##      echo '        Call.callStateId != 3   and                              ' >> $SQL_FILE
        echo '        CallResult.billedFlag = 0  and                           ' >> $SQL_FILE
##      echo "        Call.ani not in ('4162636300','4162636388')  and         " >> $SQL_FILE
        echo "   callStartTime between '${LASTRUN_DATE}' and '${TO_DATE}' and  " >> $SQL_FILE
        echo "   totalPrice is NOT NULL                                        " >> $SQL_FILE
...
if [ ${ERROR_CODE} -ne 0 ]; then
   echo "Step 4 - checking table skipped." >> ${MAIL_FILE}
	...
	# ... compose sql script
	echo 'if exists ( select 1 from dbo.FtpSCBUnbillable' > ${SQL_FILE}
	echo '            where  call900Number is NULL)' >> ${SQL_FILE}
	echo 'begin' >> ${SQL_FILE}
	...
	ERROR_CODE=$?
        ...
        if [ ${NotFound} -gt 0 ]; then
          echo '' >> ${MAIL_FILE}
	  echo "Step 4a - Non-matching SCB number exists - ignore this error  ..." >> ${MAIL_FILE}
##          ERROR_CODE=999
        else
          echo '' >> ${MAIL_FILE}
...
########################################################
# step 7 - BCP out SCB File to be ftp'd to vendor
########################################################
...
use ${ARCHIVE_DB} 
go
select count(*) from ${FTP_ARCHIVE_TABLE} 
...
echo "use ${ARCHIVE_DB}                                                "  > $SQL_FILE
echo 'go                                                               ' >> $SQL_FILE
echo 'exec p_update_FtpSCBBatchLogUnbill ' ${BATCH_ID} ",'${TODAYS_DATE}','SUCCESSFULL'," ${ARCHIVE_ROWS}  >> $SQL_FILE
echo 'go                                                               ' >> $SQL_FILE
# ... run sql
run_sql isql ${ARCHIVE_SERVER} ${ARCHIVE_DB} cron_sa ${ARCHIVE_PASSWORD} \
${SQL_FILE} ${OUT_FILE} ${TMP_FILE} ${MAIL_FILE}
...
if [ ${ERROR_CODE} -ne 0 ]; then
    ERROR_CODE=999
    echo "... Failed updating record in FtpSCBBatchLogUnbillable table, error ${ERROR_CODE}." >> ${MAIL_FILE}
fi
fi
