...
if exists (select null from FtpSCBCallReceivedHistory where fileName = '${REC_FILENAME}')
begin
delete from FtpSCBCallReceivedHistory where fileName='${REC_FILENAME}'
end
go
...
where  receivedBackFlag  = 1       and 
      (processedFlag     = 0 or processedFlag = NULL)  and
       ...
       and fileName = "20100113.SCB.CAD.ftp"
go
exit  
EOF1`"
