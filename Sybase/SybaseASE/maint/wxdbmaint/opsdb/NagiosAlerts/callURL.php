#!/usr/bin/php -q
<?php
//slight mods september 15, 2011, posting to fn-ostickets
ini_set("allow_url_fopen",1);
    ...
    if (isset($argv[1]))
    {
        if ($argv[1] == "test")
        {
            $subject = urlencode("test backup_q ticket");
            $message = urlencode("here is some test text");
            $nocrequest = file_get_contents("https://noc.frontiernetworks.ca/class_ticket.php?q=API-llqueue&subject={$subject}&content={$message}");
            echo PHP_EOL.$nocrequest.PHP_EOL;
            exit;
        }
    }
...
$nocrequest = file_get_contents("https://noc.frontiernetworks.ca/class_ticket.php?q=API-llqueue&subject={$subjectesc}&content={$contentesc}");
