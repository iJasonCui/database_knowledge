...
GenerateSQL()
{
  ...
  echo "go"                                                                    >> ${SQLdumpdb}
  echo "CHECKPOINT"                                                            >> ${SQLdumpdb}
  echo "go"                                                                    >> ${SQLdumpdb}
  echo "DUMP TRAN ${DB} WITH no_log"                                           >> ${SQLdumpdb}
  echo "go"                                                                    >> ${SQLdumpdb}
  echo ""                                                                      >> ${SQLdumpdb}
...
dumpDatabase()
{
   ...
   ##
   if [ $RC = 0 ]; then
       #egrep -v "Backup Server" /tmp/sql01.$$ | egrep "error|ERROR|failed|FAILED|Msg|Server"  >/tmp/err01.$$
       sed 's/Backup Server//g'  /tmp/sql01.$$ | egrep "error|ERROR|failed|FAILED|Msg|Server"  >/tmp/err01.$$
