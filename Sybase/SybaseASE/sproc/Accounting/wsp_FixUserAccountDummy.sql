IF OBJECT_ID('dbo.wsp_FixUserAccountDummy') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.wsp_FixUserAccountDummy
    IF OBJECT_ID('dbo.wsp_FixUserAccountDummy') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.wsp_FixUserAccountDummy >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.wsp_FixUserAccountDummy >>>'
END
go
CREATE PROCEDURE dbo.wsp_FixUserAccountDummy
AS
BEGIN
...
--UPDATE UserAccount set purchaseOfferId=1,dateModified=@dateCreated where purchaseOfferId=9
--UPDATE UserAccount set purchaseOfferId=3,dateModified=@dateCreated where purchaseOfferId=10

SELECT userId, purchaseOfferId
INTO #FixUserAccount00655
FROM UserAccount 
WHERE purchaseOfferId = 9 or purchaseOfferId = 10
...
FETCH CUR_FixUserAccount00655 INTO @userId, @purchaseOfferId
...
go
IF OBJECT_ID('dbo.wsp_FixUserAccountDummy') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.wsp_FixUserAccountDummy >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.wsp_FixUserAccountDummy >>>'
go
EXEC sp_procxmode 'dbo.wsp_FixUserAccountDummy','unchained'
go
GRANT EXECUTE ON dbo.wsp_FixUserAccountDummy TO web
go
