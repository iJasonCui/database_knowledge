IF OBJECT_ID('dbo.wsp_purchaseCredit2') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.wsp_purchaseCredit2
    IF OBJECT_ID('dbo.wsp_purchaseCredit2') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.wsp_purchaseCredit2 >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.wsp_purchaseCredit2 >>>'
END
go
/******************************************************************************
**
...
**
******************************************************************************/
CREATE PROCEDURE dbo.wsp_purchaseCredit2
@userId NUMERIC(12,0),
@xactionId INT,
...
BEGIN TRAN TRAN_purchaseCredit2
     ...
     IF @adminUserId > 0
        BEGIN
           ...
           IF @@error > 0
	       	  BEGIN
                 ROLLBACK TRAN TRAN_purchaseCredit2
                 RETURN 98
              END
        END
                 ...
                 BEGIN
                   IF EXISTS (SELECT 1 FROM CreditBalance WHERE userId = @userId AND creditTypeId = @creditTypeId)
                      BEGIN
                            ...
                            IF @@error = 0
                                BEGIN
                                    COMMIT TRAN TRAN_purchaseCredit2
                                    SELECT @xactionId
                                    RETURN 0
                                END
                            ELSE
...
go
IF OBJECT_ID('dbo.wsp_purchaseCredit2') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.wsp_purchaseCredit2 >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.wsp_purchaseCredit2 >>>'
go
GRANT EXECUTE ON dbo.wsp_purchaseCredit2 TO web
go
