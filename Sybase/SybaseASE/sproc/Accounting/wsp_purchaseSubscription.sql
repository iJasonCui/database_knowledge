IF OBJECT_ID('dbo.wsp_purchaseSubscription') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.wsp_purchaseSubscription
    IF OBJECT_ID('dbo.wsp_purchaseSubscription') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.wsp_purchaseSubscription >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.wsp_purchaseSubscription >>>'
END
go
/******************************************************************************
**
...
**
******************************************************************************/
CREATE PROCEDURE dbo.wsp_purchaseSubscription
@userId NUMERIC(12,0),
@xactionId INT,
...
BEGIN TRAN TRAN_purchaseSubscription
     ...
     IF @adminUserId > 0
        BEGIN
           ...
           IF @@error > 0
	      BEGIN
                 ROLLBACK TRAN TRAN_purchaseSubscription
                 RETURN 98
              END
        END
	  ...
	  BEGIN
        ...
        INSERT INTO Purchase 
         ...
         IF @@error = 0
                 BEGIN
                    COMMIT TRAN TRAN_purchaseSubscription
                    SELECT @xactionId
                    RETURN 0
                 END
         ELSE
...
go
IF OBJECT_ID('dbo.wsp_purchaseSubscription') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.wsp_purchaseSubscription >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.wsp_purchaseSubscription >>>'
go
GRANT EXECUTE ON dbo.wsp_purchaseSubscription TO web
go
