IF OBJECT_ID('dbo.wsp_processAutomatedChargeback') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.wsp_processAutomatedChargeback
    IF OBJECT_ID('dbo.wsp_processAutomatedChargeback') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.wsp_processAutomatedChargeback >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.wsp_processAutomatedChargeback >>>'
END
go
/******************************************************************************
**
...
**
******************************************************************************/
CREATE PROCEDURE dbo.wsp_processAutomatedChargeback
...
BEGIN
    ...
    SELECT creditCardId INTO #processAutomatedChargeback FROM CreditCard WHERE encodedCardNum =  @encodedCardNumber
    ...
    SELECT @refXactionId=xactionId, @creditCardId = creditCardId, @userId=userId FROM Purchase WHERE
               creditCardId IN (SELECT creditCardId FROM #processAutomatedChargeback) AND
               cost + tax = @cost AND
               ...
               xactionId NOT IN (
                  ...
                  dateCreated >= @fromDateCreated AND
                  creditCardId IN (SELECT creditCardId FROM #processAutomatedChargeback))
    SET ROWCOUNT 0
...
go
IF OBJECT_ID('dbo.wsp_processAutomatedChargeback') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.wsp_processAutomatedChargeback >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.wsp_processAutomatedChargeback >>>'
go
GRANT EXECUTE ON dbo.wsp_processAutomatedChargeback TO web
go
