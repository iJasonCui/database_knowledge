IF OBJECT_ID('dbo.wsp_newUserReg') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.wsp_newUserReg
    IF OBJECT_ID('dbo.wsp_newUserReg') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.wsp_newUserReg >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.wsp_newUserReg >>>'
END
go
/******************************************************************************
**
...
**
******************************************************************************/
CREATE PROCEDURE wsp_newUserReg
 @userId               NUMERIC(12,0)
    ...
    ELSE
        BEGIN
            ...
            BEGIN TRAN TRAN_newUserReg
                ...
                IF @@error = 0
                    BEGIN
                        COMMIT TRAN TRAN_newUserReg
                        SELECT @userId AS userId
                      END
                ELSE
                    BEGIN
                        ROLLBACK TRAN TRAN_newUserReg
                    END
        END
END
go
...
IF OBJECT_ID('dbo.wsp_newUserReg') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.wsp_newUserReg >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.wsp_newUserReg >>>'
go

EXEC sp_procxmode 'dbo.wsp_newUserReg','unchained'
go

GRANT EXECUTE ON dbo.wsp_newUserReg TO web
go
