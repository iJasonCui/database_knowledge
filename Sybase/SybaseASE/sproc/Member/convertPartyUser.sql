...
LOCK ALLPAGES
go
IF OBJECT_ID('tempdb.dbo.PartyProfileCity') IS NOT NULL
    PRINT '<<< CREATED TABLE dbo.PartyProfileCity >>>'
ELSE
    PRINT '<<< FAILED CREATING TABLE dbo.PartyProfileCity >>>'
go


IF OBJECT_ID('dbo.convertPartyUser') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.convertPartyUser
    IF OBJECT_ID('dbo.convertPartyUser') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.convertPartyUser >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.convertPartyUser >>>'
END
go
CREATE PROCEDURE convertPartyUser
AS
DECLARE @cityId INT
,@jurisId SMALLINT
...
BEGIN
   ...
   BEGIN
     ...
     BEGIN
       ...
       FROM City --LegacyCity Jason Sep 1 2004
...
go
IF OBJECT_ID('dbo.convertPartyUser') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.convertPartyUser >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.convertPartyUser >>>'
go
EXEC sp_procxmode 'dbo.convertPartyUser','unchained'
go
