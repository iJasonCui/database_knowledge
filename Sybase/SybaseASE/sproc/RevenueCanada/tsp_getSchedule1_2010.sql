IF OBJECT_ID('dbo.tsp_getSchedule1_2010') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.tsp_getSchedule1_2010
    IF OBJECT_ID('dbo.tsp_getSchedule1_2010') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.tsp_getSchedule1_2010 >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.tsp_getSchedule1_2010 >>>'
END
go
CREATE PROCEDURE dbo.tsp_getSchedule1_2010
     @col260  numeric(12,2), 
     ...
     @childrenAmount  numeric(12,2), --children amount; Line 5, col367; 2 * $2101
...
BEGIN
     ...
     SELECT @col300= 10382
     SELECT @col308= @CPP --MAX 2163.15
     SELECT @col312= @EI --MAX 747.36
     --SELECT @col367= 2 * 2101
     SELECT @col367= @childrenAmount 
     SELECT @col363= 1051 --canada employment 
     SELECT @col364= 1090 --10 months
     SELECT @col365= 2 * 500 
     --SELECT @col368= 10000 -- home renovation expense schedule 12 only for 2009
     SELECT @col315= 4223 -- caregiver amount schedule 5 changjiu only for now; kairong tax year 2011
     ...
     SELECT @col335 = @col300 + @col303 + @col367 + @col308  + @col312 + @col363 + @col364 + @col365  + @col315
     ...
     IF @col260 <= 40970                       SELECT @LINE31 = convert(numeric(12,2), @col260 * 0.15)
     IF @col260 > 40970 AND @col260 <= 81941   SELECT @LINE31 = convert(numeric(12,2), 6146 + (@col260- 40970) * 0.22)
     IF @col260 > 81941 AND @col260 <= 127021
     BEGIN
          DECLARE @LINE29 NUMERIC(12,2)
          SELECT @LINE29 = @col260
          
          PRINT "Line 29 : %1! " , @LINE29
          PRINT "Line 30 : %1! " , @LINE29          
          
          DECLARE @LINE32 NUMERIC(12,2)
          SELECT @LINE32 = @col260 - 81941
          PRINT "Line 32 : %1! " , @LINE32

          DECLARE @LINE34 NUMERIC(12,2)
          SELECT @LINE34 = convert(numeric(12,2),  (@col260 - 81941) * 0.26) 
          PRINT "Line 34 : %1! " , @LINE34
          

          SELECT @LINE36 = 15159 + @LINE34

          
     END
               
     IF @col260 > 127021    SELECT @LINE36 = convert(numeric(12,2), 26880 + (@col260 - 127021) * 0.29)

     PRINT "Line 36 : %1! " , @LINE36
     PRINT "Line 37 : %1! " , @LINE36
     PRINT "Line 39 : %1! " , @LINE36
     
     DECLARE @line40  numeric(12,2)    
     PRINT "Line 40: %1! " , @col350
                   
     -- calculate the Line 38                    
     SELECT @col420  = @LINE36 - @col350 
...
go
GRANT EXECUTE ON dbo.tsp_getSchedule1_2010 TO web
go
IF OBJECT_ID('dbo.tsp_getSchedule1_2010') IS NOT NULL
   PRINT '<<< CREATED PROCEDURE dbo.tsp_getSchedule1_2010 >>>'
ELSE
   PRINT '<<< FAILED CREATING PROCEDURE dbo.tsp_getSchedule1_2010 >>>'
go
EXEC sp_procxmode 'dbo.tsp_getSchedule1_2010','unchained'
go
