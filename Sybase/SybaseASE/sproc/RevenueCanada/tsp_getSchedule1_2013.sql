IF OBJECT_ID('dbo.tsp_getSchedule1_2013') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.tsp_getSchedule1_2013
    IF OBJECT_ID('dbo.tsp_getSchedule1_2013') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.tsp_getSchedule1_2013 >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.tsp_getSchedule1_2013 >>>'
END
go
CREATE PROCEDURE dbo.tsp_getSchedule1_2013
     @col260  numeric(12,2), 
...
BEGIN
     ...
     SELECT @col312= @EI  --MAX 891.12 
     --SELECT @col367= 2 * 2191
     SELECT @col367= @childrenAmount 
     SELECT @col363= 1117 --canada employment 
     SELECT @col364= 1285 --10 months
     ...
     SELECT @col315= 2 * 4490 -- caregiver amount schedule 5 changjiu only for now; kairong tax year 2011
     ...
     IF @col260 <= 43561                       SELECT @LINE39 = convert(numeric(12,2), @col260 * 0.15)
     IF @col260 >  43561 AND @col260 <= 87123  SELECT @LINE39 = convert(numeric(12,2), 6534 + (@col260 - 43561) * 0.22)
     IF @col260 >  87123 AND @col260 <= 135054 
     BEGIN
          ...
          SELECT @LINE35 = @col260 - 87123
          PRINT "Line 35 : %1! " , @LINE35

          DECLARE @LINE37 NUMERIC(12,2)
          SELECT @LINE37 = convert(numeric(12,2),  (@col260 - 87123) * 0.26) 
          PRINT "Line 37 : %1! " , @LINE37
        

          SELECT @LINE39 = 16118 + @LINE37

          
     END
               
     IF @col260 > 135054    SELECT @LINE39 = convert(numeric(12,2), 28580 + (@col260 - 135054) * 0.29)
...
go
GRANT EXECUTE ON dbo.tsp_getSchedule1_2013 TO web
go
IF OBJECT_ID('dbo.tsp_getSchedule1_2013') IS NOT NULL
   PRINT '<<< CREATED PROCEDURE dbo.tsp_getSchedule1_2013 >>>'
ELSE
   PRINT '<<< FAILED CREATING PROCEDURE dbo.tsp_getSchedule1_2013 >>>'
go
EXEC sp_procxmode 'dbo.tsp_getSchedule1_2013','unchained'
go
