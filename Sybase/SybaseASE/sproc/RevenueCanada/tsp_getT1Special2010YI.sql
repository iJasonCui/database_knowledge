IF OBJECT_ID('dbo.tsp_getT1Special2010YI') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.tsp_getT1Special2010YI
    IF OBJECT_ID('dbo.tsp_getT1Special2010YI') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.tsp_getT1Special2010YI >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.tsp_getT1Special2010YI >>>'
END
go
CREATE PROCEDURE tsp_getT1Special2010YI
...
BEGIN
     ...
     EXEC tsp_getSchedule1_2010YI @line260, @spouseNetIncome, @combineFlag, @CPP, @EI, @Donation, @childrenAmount, @line420 output, @col345 output, @col347 output
     PRINT "Line 420: %1! " , @line420
     
      PRINT "#-----------------------------------------#" 
      PRINT "Go to Ontario Tax 428"
      --ontario tax 428
      EXEC tsp_getON428_2010YI @line260,   @spouseNetIncome,   @combineFlag , @CPP, @EI, @col345, @col347, @line428 output 
...
go
GRANT EXECUTE ON dbo.tsp_getT1Special2010YI TO web
go
IF OBJECT_ID('dbo.tsp_getT1Special2010YI') IS NOT NULL
   PRINT '<<< CREATED PROCEDURE dbo.tsp_getT1Special2010YI >>>'
ELSE
   PRINT '<<< FAILED CREATING PROCEDURE dbo.tsp_getT1Special2010YI >>>'
go
EXEC sp_procxmode 'dbo.tsp_getT1Special2010YI','unchained'
go
