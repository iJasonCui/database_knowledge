IF OBJECT_ID('dbo.tsp_getSchedule1_2011') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.tsp_getSchedule1_2011
    IF OBJECT_ID('dbo.tsp_getSchedule1_2011') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.tsp_getSchedule1_2011 >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.tsp_getSchedule1_2011 >>>'
END
go
CREATE PROCEDURE dbo.tsp_getSchedule1_2011
     @col260  numeric(12,2), 
     ...
     @childrenAmount  numeric(12,2), --children amount; Line 5, col367; 2 * $2131
...
BEGIN
     ...
     SELECT @col363= 1065 --canada employment 
     SELECT @col364= 1260 --10 months
     SELECT @col365= 2 * 500  -- fitness amount
     SELECT @col370= 2 * 500  --art amount
     --SELECT @col368= 10000 -- home renovation expense schedule 12 only for 2009
     SELECT @col315= 2 * 4282 -- caregiver amount schedule 5 changjiu only for now; kairong tax year 2011
     ...
     IF @col260 <= 41544                       SELECT @LINE39 = convert(numeric(12,2), @col260 * 0.15)
     IF @col260 >  41544 AND @col260 <= 83088  SELECT @LINE39 = convert(numeric(12,2), 6232 + (@col260 - 41544) * 0.22)
     IF @col260 >  83088 AND @col260 <= 128800 
     BEGIN
          ...
          SELECT @LINE35 = @col260 - 83088
          PRINT "Line 35 : %1! " , @LINE35

          DECLARE @LINE37 NUMERIC(12,2)
          SELECT @LINE37 = convert(numeric(12,2),  (@col260 - 83088) * 0.26) 
          PRINT "Line 37 : %1! " , @LINE37
        

          SELECT @LINE39 = 15371 + @LINE37

          
     END
               
     IF @col260 > 127021    SELECT @LINE39 = convert(numeric(12,2), 27256 + (@col260 - 128088) * 0.29)
...
go
GRANT EXECUTE ON dbo.tsp_getSchedule1_2011 TO web
go
IF OBJECT_ID('dbo.tsp_getSchedule1_2011') IS NOT NULL
   PRINT '<<< CREATED PROCEDURE dbo.tsp_getSchedule1_2011 >>>'
ELSE
   PRINT '<<< FAILED CREATING PROCEDURE dbo.tsp_getSchedule1_2011 >>>'
go
EXEC sp_procxmode 'dbo.tsp_getSchedule1_2011','unchained'
go
