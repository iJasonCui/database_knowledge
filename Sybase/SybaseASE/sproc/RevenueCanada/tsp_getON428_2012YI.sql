IF OBJECT_ID('dbo.tsp_getON428_2012YI') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.tsp_getON428_2012YI
    IF OBJECT_ID('dbo.tsp_getON428_2012YI') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.tsp_getON428_2012YI >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.tsp_getON428_2012YI >>>'
END
go
CREATE PROCEDURE tsp_getON428_2012YI
     @col260  numeric(12,2), 
...
BEGIN
     ...
     SELECT @col5804  = 9405
     SELECT @col5804_1= 8784
     SELECT @col5840  = 0 --2 * 4433
     ...
     BEGIN
          ...
          IF @col5812 >= 7986 SELECT @col5812 = 7986
     END
     ELSE BEGIN
          SELECT @col5812 = 0
     END
     ...
     IF @col260 <= 39020                       SELECT @LINE36 = convert(numeric(12,2), @col260 * 0.0505)
     IF @col260 >  39020 AND @col260 <= 78043  SELECT @LINE36 = convert(numeric(12,2), 1971 + (@col260 - 39020) * 0.0915)
     IF @col260 >  78043                       
     BEGIN
     ...
     --ontario surtax
     SELECT @line50  = convert(numeric(12,2), (@line49 - 4213) *0.2 )
     IF  @line50  < 0  SELECT @line50  =  0
     PRINT "Line 50 %1! " , @line50
      
     DECLARE @line51 numeric(12,2)
     SELECT @line51  = convert(numeric(12,2), (@line49 - 5392) *0.36 )
...
go
GRANT EXECUTE ON dbo.tsp_getON428_2012YI TO web
go
IF OBJECT_ID('dbo.tsp_getON428_2012YI') IS NOT NULL
   PRINT '<<< CREATED PROCEDURE dbo.tsp_getON428_2012YI >>>'
ELSE
   PRINT '<<< FAILED CREATING PROCEDURE dbo.tsp_getON428_2012YI >>>'
go
EXEC sp_procxmode 'dbo.tsp_getON428_2012YI','unchained'
go
