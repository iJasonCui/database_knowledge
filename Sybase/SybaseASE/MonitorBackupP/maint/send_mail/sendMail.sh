#!/bin/ksh
####################################################################################
# this ksh scripts is created for processing Monitor Backup email								   #
...
echo "From:Backup Monitor" >> ${LogFile}
...
$SYBASE/$SYBASE_OCS/bin/sqsh -w10000 -S${Server} -h  -Usa -P${Password} >> ${LogFile} <<EOF1
