...
#--------------------------------------------------------
if [  $# -lt 1 -o  $# -gt 3 ]; then
   echo  "Usage: emailMDAStats.sh <ASE ServerName> "
   exit 1
fi
...
print '-----------------------------------------------------------------------'
print 'Top 20 Physical Read (expensive) Stored Procedures               '
print '-----------------------------------------------------------------------'
print '  '
go
select top 20 ProcName, DBName, PhysicalReads, StartTime
from mda_user.proc_stats
where SRVName      = '${aseServer}'
  and dateCreated >= '${startDate}'
  and dateCreated < '${enDate}'
...
from mda_user.proc_stats
where SRVName      = '${aseServer}'
  and dateCreated >= dateadd(dd, -1, convert(datetime, '${startDate}'))
  and dateCreated <  dateadd(dd, -1, convert(datetime, '${enDate}'))
group by DBName, ProcName
