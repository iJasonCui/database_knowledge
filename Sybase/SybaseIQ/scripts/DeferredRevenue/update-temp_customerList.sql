...
BEGIN
    DROP INDEX temp_jason_BalanceLastPurchase.idx_userId
    IF EXISTS (SELECT * FROM sysindexes WHERE id=OBJECT_ID('dbo.temp_jason_BalanceLastPurchase') AND name='idx_userId')
        PRINT '<<< FAILED DROPPING INDEX temp_jason_BalanceLastPurchase.idx_userId >>>'
    ELSE
        PRINT '<<< DROPPED INDEX temp_jason_BalanceLastPurchase.idx_userId >>>'
END
go
