...
then
        ...
        filename5=$logical.subscription
else
        ...
        filename5=$server.$database.subscription
fi
...
echo "Instructions to create subscription in $filename5"
echo ""
...
pagesize=`isql $username $password $dsname -b -Q -X --conceal << EOI
set nocount on
go
select @@maxpagesize
go
...
while (select  min(id) from #table_list where id > @tabid) != NULL
        ...
        print @msg
        select @msg = 'with primary table named  ' + @tablestr
        print @msg
        select @msg = 'with replicate table named ' + @tablestr
        print @msg
        print "("
...
grep "drop" logicalSRV.$database.droprepdefs  | sed -e "s/drop replication definition //" | sed -e "s/$suffix//g" | sed -e "s/_/ /g"   > /tmp/subsc.$$

echo "" > $filename5
echo "" > logicalSRV.$database.droprepsubs
echo "" > logicalSRV.$database.checkrepsubs

server=`echo $dsname |  sed -e "s/-S//"`
while read PF DB TB
do
DATA_SERVER_DATABASE='PrePRODQCluster.CRMR'

echo ""
echo "define subscription ${PF}_${DB}_sub    for ${PF}_${DB}$suffix with replicate at ${DATA_SERVER_DATABASE}  "  >> $filename5
echo "go"  >> $filename5
echo "check  subscription ${PF}_${DB}_sub    for  ${PF}_${DB}$suffix with replicate at ${DATA_SERVER_DATABASE} "  >> $filename5
echo "go"  >> $filename5
echo "activate subscription ${PF}_${DB}_sub  for  ${PF}_${DB}$suffix with replicate at ${DATA_SERVER_DATABASE} "  >> $filename5
echo "go"  >> $filename5
echo "check  subscription ${PF}_${DB}_sub    for  ${PF}_${DB}$suffix with replicate at ${DATA_SERVER_DATABASE} "  >> $filename5
echo "go"  >> $filename5
echo "validate subscription ${PF}_${DB}_sub  for  ${PF}_${DB}$suffix with replicate at ${DATA_SERVER_DATABASE} "  >> $filename5
echo "go"  >> $filename5
echo "check  subscription ${PF}_${DB}_sub    for  ${PF}_${DB}$suffix with replicate at ${DATA_SERVER_DATABASE} "  >> $filename5
echo "go"  >> $filename5
echo " "  >> $filename5
...
echo "drop subscription ${PF}_${DB}_sub    for ${PF}_${DB}$suffix with replicate at ${DATA_SERVER_DATABASE}   without purge" >> logicalSRV.$database.droprepsubs
echo "go" >> logicalSRV.$database.droprepsubs
echo "check subscription ${PF}_${DB}_sub    for ${PF}_${DB}$suffix with replicate at ${DATA_SERVER_DATABASE} " >> logicalSRV.$database.checkrepsubs
echo "go" >> logicalSRV.$database.checkrepsubs


done < /tmp/subsc.$$

grep drop logicalSRV.$database.droprepdefs    | sed -e "s/drop replication definition //" | sed -e "s/_/ /g" > $database.subscript.cfg
