...
objectKey in (select objectKey from succor.audit.tObjectConfigArchive where schemaName in ('web'))
and dateKey = 'Aug 15 2012'
--commit tran jason

select * from succor.audit.tLoadArchive where objectKey in (
select objectKey from succor.audit.tObjectConfigArchive where schemaName in ('web'))
and dateKey in ('Aug 19 2012')
...
SELECT    objectKey, objectName, object, objectType, businessUnit
FROM      succor.audit.vObject
WHERE     (businessUnit = 'WEB') AND (objectType = 'Cube')
...
--update succor.audit.tLoadArchive 
set rowCountRaw = 0, rowCountLoaded = 0, statusKey = 2, attempts =0, 
    finishedDateTime = null, startDateTime = null where loadArchiveKey in  (462091)
...
--table
select * from [acumen].[fact].[tIvrSales] where loadTableKey = 496461
