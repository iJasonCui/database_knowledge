...
#--------------------------------------------------------------------
ARCHIVE_KEY=`fisql -U${DEST_USER} -P${DEST_PASS} -S${DEST_SRV}  <<- EOF1 | sed '1,2d' | sed 's/ //g'
set nocount on
...
# table name
#------------------------
OBJECT_NAME=`fisql -U${DEST_USER} -P${DEST_PASS} -S${DEST_SRV}  <<- EOF1 | sed '1,2d' | sed 's/ //g'
set nocount on
...
mysql -h${SOURCE_IP} -P${SOURCE_PORT} -u ${SOURCE_USER} --password=${SOURCE_PASS} < ${SQL_FILE}
...
freebcp archive.web.${OBJECT_NAME} in ${BCP_PATH}/${BCP_FILE} -U${DEST_USER} -P${DEST_PASS} -S${DEST_SRV} \
...
fisql -U${DEST_USER} -P${DEST_PASS} -S${DEST_SRV} -i ${SQL_FILE} > ${OUT_FILE} 
