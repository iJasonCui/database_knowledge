...
# accept parameters
SCRIPT_NAME="cron_arch_messages"
...
MAIL_FLAG=${5:-0}
DUMP_DB_FLAG=${6:-1}
FROM_DATE=${7:-`date --date='1 day ago' +%Y-%m-%d`}
TO_DATE=${8:-`date +%Y-%m-%d`}
...
/usr/bin/mysql -h${OUT_IP} -P${OUT_PORT} -u ${LOGIN} --password=${OUT_PASS} < ${SQL_FILE} 
...
ROWCOUNT=`cat ${OUT_FILE}`

if [ ${ROWCOUNT} -ne 0 ]
then
   echo "FAILED at `date`, it has been archived and rowcount is ${ROWCOUNT}." >> ${MAIL_FILE} 
   ERROR_CODE=1
fi
...
echo '' >> ${MAIL_FILE}
if [ ${ERROR_CODE} -ne 0 ]; then
    echo "Stage 2 - bcp out from ${IN_SERVER}.${IN_DB}, skipped." >> ${MAIL_FILE}
else
    echo "Stage 2 - bcp out from ${IN_SERVER}.${IN_DB} ..." >> ${MAIL_FILE}
...
BCP_FILE="${SCRIPT_NAME}.bcpout.${FROM_DATE}.${TO_DATE}"

if [ -e ${BCP_DIR}/${BCP_FILE} ]
then
   rm ${BCP_DIR}/${BCP_FILE}
fi
...
echo "  id, direction, pdu , pdu_resp, sequence, country, phone_number, carrier, sub_carrier, shortcode, billed, " >> ${SQL_FILE}
echo "  datetime_inserted, datetime_modified,  status , status_message, ticket_id , session_id , attempts, " >> ${SQL_FILE}
echo "  must_resend, completed, failed " >> ${SQL_FILE}
echo "INTO OUTFILE '${BCP_DIR}/${BCP_FILE}' " >> ${SQL_FILE}
...
echo '' >> ${MAIL_FILE}
if [ ${ERROR_CODE} -ne 0 ]; then
    echo "Stage 3 - load data into ${OUT_SERVER}.${OUT_DB} , skipped." >> ${MAIL_FILE}
else
    echo "Stage 3 - load data into ${OUT_SERVER}.${OUT_DB} ..." >> ${MAIL_FILE}
...
echo "LOAD DATA LOCAL INFILE  " > ${SQL_FILE}
echo " '${BCP_DIR}/${BCP_FILE}' " >> ${SQL_FILE}
echo " INTO TABLE arch_sms_gateway.messages " >> ${SQL_FILE}
echo " FIELDS TERMINATED BY '|~|'  " >> ${SQL_FILE}
echo " LINES TERMINATED BY '|@|\n' " >> ${SQL_FILE}
echo " (  " >> ${SQL_FILE}
echo "  id, direction, pdu , pdu_resp, sequence, country, phone_number, carrier, sub_carrier, shortcode, billed, " >> ${SQL_FILE}
